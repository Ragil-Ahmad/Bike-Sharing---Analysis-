...
def create_weekday_avg(day_df):
    weekday_avg = day_df.groupby('weekday')[['registered', 'casual']].mean().mean()
    return weekday_avg

def create_workingday_avg(day_df):
    workingday_avg = day_df.groupby('workingday')[['registered', 'casual']].mean().mean()
    return workingday_avg

def create_holiday_avg(day_df):
    holiday_avg = day_df.groupby('holiday')[['registered', 'casual']].mean().mean()
    return holiday_avg
...
def create_casual_user(day_df):
    casual_user = day_df.groupby(by='date').agg({
        'casual': 'sum'
    }).reset_index()
    return casual_user

def create_registered_user(day_df):
    registered_user = day_df.groupby(by='date').agg({
        'registered': 'sum'
    }).reset_index()
    return registered_user

def create_daily_user_count(day_df):
    daily_user_count = day_df.groupby(by='date').agg({
        'count': 'sum'
    }).reset_index()
...
# Create groupings
season_weather_group = main_df_day.groupby('season')['count'].mean().reset_index()
...
st.subheader("Apakah perubahan musim berpengaruh pada penyewaan sepeda ?")
...
ax.set_title('Average Count by Season', fontsize=14)
